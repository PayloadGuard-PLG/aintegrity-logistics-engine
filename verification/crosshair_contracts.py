...
#   p6_gain_bounded       : gain ≤ METRIC_CAP − start_metric
#   p8_gain_mono_budget   : budget₁ ≥ budget₂ ≥ 0 → gain(budget₁) ≥ gain(budget₂)
#   p9_gain_mono_mult     : mult₁ ≥ mult₂ > 0 → gain(mult₁) ≥ gain(mult₂)
#   p16_decay_nonneg      : apply_periodic_degradation never produces negative metrics
#   p17_decay_mono_levels : more periods → lower or equal metric values

import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

from verification.engine_pure import metric_gain_from_budget, apply_periodic_degradation, apply_intervention
from verification.constants_pure import METRIC_CAP


def p5_gain_nonneg(start_metric: float, budget: float, mult: float) -> None:
    """
    pre: 0.0 <= start_metric <= 9999.0
    pre: budget > 0.0
    pre: mult > 0.0
    """
    result = metric_gain_from_budget(start_metric, budget, mult)
    assert result >= 0.0, f"P5 violated: gain={result}"


def p6_gain_bounded(start_metric: float, budget: float, mult: float) -> None:
    """
    pre: 0.0 <= start_metric <= 9999.0
    pre: budget >= 0.0
    pre: mult > 0.0
    """
    result = metric_gain_from_budget(start_metric, budget, mult)
    assert result <= METRIC_CAP - start_metric, f"P6 violated: gain={result}"


def p8_gain_mono_budget(start_metric: float, budget1: float, budget2: float, mult: float) -> None:
    """
    pre: 0.0 <= start_metric <= 9999.0
    pre: budget1 >= budget2 >= 0.0
    pre: mult > 0.0
    """
    g1 = metric_gain_from_budget(start_metric, budget1, mult)
    g2 = metric_gain_from_budget(start_metric, budget2, mult)
    assert g1 >= g2, f"P8 violated: gain({budget1})={g1} < gain({budget2})={g2}"


def p9_gain_mono_mult(start_metric: float, budget: float, mult1: float, mult2: float) -> None:
    """
    pre: 0.0 <= start_metric <= 9999.0
    pre: mult1 >= mult2 > 0.0
    pre: budget >= 0.0
    """
    g1 = metric_gain_from_budget(start_metric, budget, mult1)
    g2 = metric_gain_from_budget(start_metric, budget, mult2)
...
def p16_decay_nonneg(values: list, levels: int, decay_per: float) -> None:
    ...
    """
    result = apply_periodic_degradation(values, levels, decay_per)
    for i, v in enumerate(result):
        assert v >= 0.0, f"P16 violated: metric[{i}]={v} < 0"
...
def p17_decay_mono_levels(values: list, levels1: int, levels2: int, decay_per: float) -> None:
    ...
    pre: 0.0 <= decay_per <= 100.0
    """
    after1 = apply_periodic_degradation(values, levels1, decay_per)
    after2 = apply_periodic_degradation(values, levels2, decay_per)
    for i in range(len(values)):
        assert after1[i] <= after2[i], (
            f"P17 violated at index {i}: periods={levels1} gives {after1[i]}"
            f" > periods={levels2} gives {after2[i]}"
        )
