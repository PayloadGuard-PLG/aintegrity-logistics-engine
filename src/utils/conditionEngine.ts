...
// These are SEPARATE from profile.cycleIntensityMultipliers (which scale XP gain only).
// Formula: conditionLoss = baseLossPerDrill × condLevelMultipliers[level] × (1 − fanClubCondReduction[fanLevel])
export const COND_LEVEL_MULTIPLIERS: Record<string, number> = profile.intensityMultipliers;
...
const FAN_RETENTION: number[] = profile.supportDrainReduction.map(r => 1 - r);

// Returns per-drill condition loss %.
// baseLoss = profile.baseDrainPerCycle (0.75) × difficulty multiplier × fan club retention.
